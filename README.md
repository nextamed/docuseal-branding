...
4. **Neutrale Titel/Metas** (`overrides/app/views/layouts/_head_tags.html.erb`, `overrides/app/views/shared/_meta.html.erb`) — Browser-Tab-Titel „OKorn Immobilien — Signatur-Plattform" statt DocuSeal-Produktname; OpenGraph/Description neutralisiert, Twitter-Handles entfernt, `og:site_name` = OKorn Immobilien. Favicon-Links unveraendert. Der Signing-Tab-Titel kommt aus `submit_form/show.html.erb` („… | OKorn Immobilien").
   **Suchmaschinen-Ausschluss:** `_meta.html.erb` setzt `<meta name="robots" content="noindex, nofollow, noarchive, nosnippet, noimageindex">` auf **jeder** Seite — Upstream nimmt Root-Pfad und Devise-Seiten (Login, Passwort-Reset) aus, also genau die Surfaces, die ein Crawler ohne Signier-Link erreicht. Dazu `overrides/public/robots.txt` mit `Allow: /` und bewusst **ohne** `Disallow`: eine per robots.txt gesperrte URL wird nicht gecrawlt, das `noindex` also nie gelesen, und Google darf sie als URL-only-Treffer (inkl. Signier-Slug im Pfad) indexieren. Nur `Allow` + `noindex` haelt die Domain garantiert aus dem Index. Verifikation nach Deploy: `curl -sS https://sign.okorn.biz/ | grep -i '<meta name="robots"'` und `curl -sS https://sign.okorn.biz/robots.txt`.
...
2. **Vor dem Merge pruefen**: Haben sich diese Pfade upstream geaendert?
   ...
   - `app/views/pages/landing.html.erb` — **Full-File-Override!** Bei Upstream-Bumps pruefen, ob `shared/logo` noch existiert. Rendert bewusst KEINE Attribution (kein Pflicht-Surface; die vier Empfaenger-Surfaces behalten ihre).
   - `app/views/layouts/_head_tags.html.erb` + `app/views/shared/_meta.html.erb` — kleine Overrides; bei Upstream-Bumps gegen neue Meta-/Title-Logik diffen. Das `noindex` in `_meta.html.erb` muss **unbedingt** bleiben (kein `if` um den Meta-Tag).
